...
function pipeline_test()
    ...
    ohe = OneHotEncoder()
    rfregressor1 = SKLearner("RandomForestRegressor")
    rfregressor2 = SKLearner("RandomForestRegressor", n_estimators=100)
    rfclassifier1 = SKLearner("RandomForestClassifier", n_estimators=100)
    rfclassifier2 = SKLearner("RandomForestClassifier")(n_estimators=100)
    plr = @pipeline (catf |> ohe) + (numf |> rb |> pca) |> rfregressor1
    plr.name = "(catf |> ohe) + (numf |> rb |> pca) |> rfregressor1"
    plr1 = (catf |> ohe) + (numf |> rb |> pca) |> rfregressor2
    plr1.name = "(catf |> ohe) + (numf |> rb |> pca) |> rfregressor2"
    plc = @pipeline (catf >> ohe) + (numf >> rb >> pca) |> rfclassifier1
    plc.name = "(catf >> ohe) + (numf >> rb >> pca) |> rfclassifier1"
    plc1 = (catf >> ohe) + (numf >> rb >> pca) |> rfclassifier2
    plc1.name = "(catf >> ohe) + (numf >> rb >> pca) |> rfclassifier2"
